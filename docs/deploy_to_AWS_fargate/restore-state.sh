#!/bin/bash
# =============================================================================
# restore-state.sh — Rebuild .hr-demo-state from live AWS resources
# =============================================================================
...
# then writes a .hr-demo-state file identical to the one deploy.sh produces.
...
# The default instance keeps the original ".hr-demo-state" name; other instances
# are namespaced ".hr-demo-state.<instance>" so several can coexist.
state_file_for() {
...
DEFAULT_INSTANCE="${INSTANCE:-hr-demo}"
if [ -n "${INSTANCE:-}" ]; then
  APP_NAME="$INSTANCE"
else
...
cat > "$STATE_FILE" <<EOF
# hrDemoWebApp deployment state — regenerated by restore-state.sh
